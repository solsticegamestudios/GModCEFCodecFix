...
impl IntegrityStatus {
	pub const fn as_str(&self) -> &'static str {
		match self {
			Self::NeedDelete => "Needs Delete",
			Self::NeedOriginal => "Needs Original + Fix",
			Self::NeedWipeFix => "Needs Wipe + Fix",
			Self::NeedFix => "Needs Fix",
			Self::Fixed => "Already Fixed",
		}
	}
}
...
fn determine_file_integrity_status(gmod_path: PathBuf, filename: &str, hashes: &HashMap<String, String>) -> Result<IntegrityStatus, String> {
	...
	if let Ok(file_path) = gmod_path
		.extend_and_return(filename.split("/"))
		.to_canonical_pathbuf(false)
	{
	...
	W: std::io::Write + 'static
{
	...
	if let Ok(cache_file_path) = cache_file_path.to_canonical_pathbuf(false) {
		if let Ok(file_hash) = get_file_hash(&cache_file_path) {
	...
	if let Some(parent_dir) = cache_file_path.parent() {
		if let Err(error) = tokio::fs::create_dir_all(parent_dir).await {
			terminal_write(
				writer,
				format!("\tFailed to Download: {filename} | Step 2: {error}").as_str(),
				true,
				writer_is_interactive.then_some("red")
			);
			return Err(());
		}
	}

	if let Some(response) = get_http_response(writer, writer_is_interactive, &PATCH_SERVER_ROOTS, filename.as_str()).await {
		let mut body = StreamReader::new(response.bytes_stream().map_err(io::Error::other));
		let mut file = match tokio::fs::File::create_new(cache_file_path).await {
			Ok(file) => file,
			Err(error) => {
				terminal_write(
					writer,
		...
		if let Err(error) = tokio::io::copy(&mut body, &mut file).await {
			terminal_write(
				writer,
				format!("\tFailed to Download: {filename} | Step 2: {error}").as_str(),
				true,
	...
	W: std::io::Write + 'static
{
	...
	// Windows
	#[cfg(windows)]
	if sys.processes_by_exact_name("gmod.exe".as_ref()).next().is_some() {
		return Err(AlmightyError::Generic("Garry's Mod is currently running. Please close it before running this tool.".to_string()));
	}

	// Linux / macOS
	#[cfg(not(windows))]
	if sys.processes_by_exact_name("gmod".as_ref()).next().is_some() {
		return Err(AlmightyError::Generic("Garry's Mod is currently running. Please close it before running this tool.".to_string()));
	}

	// Find Steam
	let steam_path = if let Some(steam_path_arg) = args.steam_path {
		// Make sure the path the user is forcing actually exists
		if let Ok(steam_path_arg) = steam_path_arg.to_canonical_pathbuf(true) {
			Some(steam_path_arg)
		} else {
		...
		#[cfg(windows)]
		{
			windows_registry::CURRENT_USER
				...
				.and_then(|steam_reg_key| steam_reg_key.get_string("SteamPath"))
				.ok()
				.and_then(|steam_path| steam_path.to_canonical_pathbuf(true).ok())
		}
	...
	terminal_write(writer, format!("Steam Path: {}\n", steam_path.display()).as_str(), true, None);

	// Get most recent Steam User, which is probably the one they're using/want
	let steam_loginusers_path = steam_path.join("config").join("loginusers.vdf");
	let Ok(steam_loginusers) = tokio::fs::read_to_string(&steam_loginusers_path).await else {
		return Err(AlmightyError::Generic("Couldn't find Steam loginusers.vdf. Have you ever launched/signed in to Steam?".to_string()));
	};
	let Ok(steam_loginusers) = Vdf::parse(&steam_loginusers) else {
	...
	// Get Steam Libraries
	let steam_libraryfolders_path = steam_path.join("steamapps").join("libraryfolders.vdf");
	let Ok(steam_libraryfolders) = tokio::fs::read_to_string(&steam_libraryfolders_path).await else {
	...
	let Ok(steam_libraryfolders) = Vdf::parse(&steam_libraryfolders) else {
	...
	for (_, steam_library) in steam_libraryfolders.value.unwrap_obj().iter() {
		...
		if steam_library_apps.contains_key(GMOD_STEAM_APPID) {
			...
			gmod_steam_library_path = steam_library_path.unwrap_str().as_ref().to_canonical_pathbuf(true).ok();
			break;
		}
	}

	let Some(gmod_steam_library_path) = gmod_steam_library_path else {
		return Err(AlmightyError::Generic("Couldn't find Garry's Mod app registration. Is Garry's Mod installed?".to_string()));
	};
	terminal_write(writer, format!("GMod Steam Library: {}\n", gmod_steam_library_path.display()).as_str(), true, None);

	// Get GMod manifest
	let gmod_manifest_path = gmod_steam_library_path.join("steamapps").join("appmanifest_4000.acf");
	let Ok(gmod_manifest) = tokio::fs::read_to_string(&gmod_manifest_path).await else {
		return Err(AlmightyError::Generic("Couldn't find GMod's appmanifest_4000.acf. Is Garry's Mod installed?".to_string()));
	};
	let Ok(gmod_manifest) = Vdf::parse(&gmod_manifest) else {
	...
	let Ok(gmod_path) = gmod_steam_library_path.join("steamapps").join("common").join(gmod_path.as_ref()).to_canonical_pathbuf(true) else {
		return Err(AlmightyError::Generic("Couldn't find Garry's Mod directory. Is Garry's Mod installed?".to_string()));
	};
	let gmod_path = gmod_path.as_path();
	terminal_write(writer, format!("GMod Path: {}\n", gmod_path.display()).as_str(), true, None);
	...
	#[cfg(not(any(windows, target_os = "macos")))]
	if let Ok(metadata) = gmod_path.metadata() {
		if metadata.uid() != 0 {
	...
	{
		// Get Steam config
		let steam_config_path = steam_path.join("config").join("config.vdf");
		let Ok(steam_config) = tokio::fs::read_to_string(&steam_config_path).await else {
			return Err(AlmightyError::Generic("Couldn't Steam config.vdf. Have you ever launched/signed in to Steam?".to_string()));
		};
		let Ok(steam_config) = Vdf::parse(&steam_config) else {
	...
	let steam_user_localconfig_path = steam_path
		.join("userdata")
		.join(steam_id.account_id().into_u32().to_string())
		.join("config")
		.join("localconfig.vdf");
	let Ok(steam_user_localconfig) = tokio::fs::read_to_string(&steam_user_localconfig_path).await else {
		return Err(AlmightyError::Generic("Couldn't find Steam localconfig.vdf. Have you ever launched/signed in to Steam?".to_string()));
	};
	let Ok(steam_user_localconfig) = Vdf::parse(&steam_user_localconfig) else {
	...
	let Some(remote_manifest_response) = get_http_response(writer, writer_is_interactive, &MANIFEST_SERVER_ROOTS, "manifest.json").await else {
	...
	let remote_manifest = remote_manifest_response
	...
	let Some(platform_branches) = remote_manifest.get(platform_masked) else {
		return Err(AlmightyError::Generic(format!("This operating system ({platform_masked}) is not supported!")));
	};

	let Some(platform_branch_files) = platform_branches.get(&gmod_branch) else {
	...
	let integrity_results: Vec<(&String, Result<IntegrityStatus, String>, &HashMap<String, String>)> = platform_branch_files.par_iter()
		.map(|(filename, hashes)| {
			let integrity_result = determine_file_integrity_status(gmod_path.to_path_buf(), filename, hashes);

			let (Ok(integrity_status) | Err(integrity_status)) = integrity_result
				.as_ref()
				.map(|integrity_status| integrity_status.as_str())
				.map_err(|error| error.as_str());

			terminal_write(
				writer,
				format!("\t{filename}: {integrity_status}").as_str(),
				true,
				(integrity_result.is_err() && writer_is_interactive).then_some("red")
			);
	...
	if pending_files_len > 0 {
		...
		let cache_dir = std::env::temp_dir().join("GModCEFCodecFix");
		if let Ok(old_cache_dir) = cache_dir.to_canonical_pathbuf(false) {
			match tokio::fs::remove_dir_all(old_cache_dir).await {
				Ok(()) => {
		...
		let cache_dir = match cache_dir.to_canonical_pathbuf(false) {
		...
		terminal_write(writer, format!("\nGModPatchTool Cache Directory: {}\n", cache_dir.display()).as_str(), true, None);

		// Download what we need
		terminal_write(writer, "Downloading patch files...".to_string().as_str(), true, None);
		...
		for (filename, integrity_status, hashes) in &pending_files {
			...
			if *integrity_status == IntegrityStatus::NeedOriginal {
				download_futures.spawn(download_file_to_cache(writer, writer_is_interactive, cache_dir.to_path_buf(), format!("originals/{platform_masked}/{gmod_branch}/{filename}"), hashes["original"].clone()));
			}
			...
			if *integrity_status != IntegrityStatus::NeedDelete {
				download_futures.spawn(download_file_to_cache(writer, writer_is_interactive, cache_dir.to_path_buf(), format!("patches/{platform_masked}/{gmod_branch}/{filename}.bsdiff"), hashes["patch"].clone()));
			}
		}

		while let Some(download_result) = download_futures.join_next().await {
			if matches!(download_result, Err(_) | Ok(Err(_))) {
		...
		.map(|(filename, integrity_status, hashes)| {
			let gmod_path = gmod_path.to_path_buf();
			terminal_write(writer, format!("\tPatching: {filename} ...").as_str(), true, None);

			let mut integrity_status = *integrity_status;
			let gmod_file_path = gmod_path.extend_and_return(filename.split("/"));
			...
			if integrity_status == IntegrityStatus::NeedDelete {
				match std::fs::remove_file(&gmod_file_path) {
					Ok(()) => {
						terminal_write(writer, format!("\tPatched: {filename}").as_str(), true, None);
						integrity_status = IntegrityStatus::Fixed;
					},
					Err(_) => {
						match std::fs::File::create(&gmod_file_path) {
							Ok(_) => {
								terminal_write(writer, format!("\tPatched: {filename}").as_str(), true, None);
								integrity_status = IntegrityStatus::Fixed;
							},
							Err(error) => {
								terminal_write(writer, format!("\tFailed to Patch: {filename} | {}: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
							}
						}
					}
				}
			...
			if integrity_status == IntegrityStatus::NeedOriginal {
				if let Ok(original_cache_file_path) = cache_dir
					.to_path_buf()
					.extend_and_return(["originals", platform_masked, gmod_branch.as_str(), filename.as_str()])
					.to_canonical_pathbuf(false)
				{
					match std::fs::copy(original_cache_file_path, &gmod_file_path) {
						Ok(_) => {
							integrity_status = IntegrityStatus::NeedFix;
						},
						Err(error) => {
							terminal_write(writer, format!("\tFailed to Patch: {filename} | {}: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
						}
			...
			if integrity_status == IntegrityStatus::NeedWipeFix {
				match File::create(&gmod_file_path) {
					Ok(_) => {
						integrity_status = IntegrityStatus::NeedFix;
					}
					Err(error) => {
			...
			if integrity_status == IntegrityStatus::NeedFix {
				let gmod_file_path = match gmod_file_path.to_canonical_pathbuf(false) {
					Ok(gmod_file_path) => gmod_file_path,
					Err(error) => {
						terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 1: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
						return (*filename, integrity_status);
					}
				};

				let patch_file_path = match cache_dir
					.to_path_buf()
					.extend_and_return(["patches", platform_masked, gmod_branch.as_str(), filename.as_str()])
					.to_canonical_pathbuf(false) {
					Ok(patch_file_path) => patch_file_path,
					Err(error) => {
						terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 2: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
						return (*filename, integrity_status);
					}
				};

				let gmod_file = match std::fs::read(&gmod_file_path) {
					Ok(gmod_file) => gmod_file,
					Err(error) => {
						terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 3: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
						return (*filename, integrity_status);
					}
				};
				...
				let patch_file = match std::fs::read(&patch_file_path) {
					...
					Err(error) => {
						terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 4: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
						return (*filename, integrity_status);
					}
				};
				...
				let patcher = match Bspatch::new(&patch_file) {
					Ok(patcher) => patcher,
					Err(error) => {
						terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 5: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
						return (*filename, integrity_status);
					}
				};

				let mut new_gmod_file = io::Cursor::new(Vec::with_capacity(patcher.hint_target_size() as usize));
				if let Err(error) =  patcher.apply(&gmod_file, &mut new_gmod_file) {
					terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 6: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
					return (*filename, integrity_status);
				}
				let new_gmod_file = new_gmod_file.into_inner();

				if let Err(error) = std::fs::write(&gmod_file_path, &new_gmod_file) {
					terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 7: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
					return (*filename, integrity_status);
				}
				...
				let file_hash = match get_file_hash(&gmod_file_path) {
					...
					Err(error) => {
						terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 8: {error}", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
						return (*filename, integrity_status);
					}
				};
				...
				if file_hash == hashes["fixed"] {
					...
					terminal_write(writer, format!("\tFailed to Patch: {filename} | {} / Step 9: Checksum mismatch", integrity_status.as_str()).as_str(), true, if writer_is_interactive { Some("red") } else { None });
				}
			}

			(*filename, integrity_status)
		}).collect();

		if patch_results.iter().map(|(_, integrity_status)| integrity_status).copied().any(|integrity_status| integrity_status != IntegrityStatus::Fixed) {
