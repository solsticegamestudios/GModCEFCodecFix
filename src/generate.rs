...
fn hash_diff_compress_file(patch_path: PathBuf, filename: &String, file_paths: &HashMap<String, PathBuf>, original_compressed_path: PathBuf) -> Result<(f64, HashMap<String, String>), (bool, String)> {
	...
	let original_hash = if let Some(original_path) = original_path { get_file_hash(original_path) } else { Ok("null".to_string()) };
	let fixed_hash = if let Some(fixed_path) = fixed_path { get_file_hash(fixed_path) } else { Ok("null".to_string()) };
	...
	if fixed_hash != "null" {
		let original = if let Some(original_path) = original_path { std::fs::read(original_path) } else { Ok(Vec::new()) };
		let fixed = if let Some(fixed_path) = fixed_path { std::fs::read(fixed_path) } else { Ok(Vec::new()) };
...
pub fn main() {
	...
	if let Err(original_path) = original_path {
		cmd.error(
			ErrorKind::InvalidValue,
			format!("Original Path (Input): {original_path}"),
		)
		.exit();
	}
	...
	if let Err(fixed_path) = fixed_path {
		cmd.error(
			ErrorKind::InvalidValue,
			format!("Fixed Path (Input): {fixed_path}"),
		)
		.exit();
	}
	...
	if let Err(patch_path) = patch_path {
		cmd.error(
			ErrorKind::InvalidValue,
			format!("Patch Path (Output): {patch_path}"),
		)
		.exit();
	}
	...
	if let Err(original_compressed_path) = original_compressed_path {
		cmd.error(
			ErrorKind::InvalidValue,
			format!("Original Compressed Path (Output): {original_compressed_path}"),
		)
		.exit();
	}
